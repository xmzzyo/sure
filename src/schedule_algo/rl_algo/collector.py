...
class Collector(object):
    ...
    def collect(
        ...
        while True:
            ...
            if random:
                ...
                if no_grad:
                    with torch.no_grad():  # faster than retain_grad version
                        ...
                        # with time_block(f"Collector {step_count}/{episode_count} policy infer."):
            ...
            # step in env
            # with time_block("Collector step"):
            obs_next, rew, done, info = self.env.step(
                action_remap, ready_env_ids)  # type: ignore
            # print("total ins: ", sum([sum(ins) for ins in self.env.ins_num]))

            each_rwd = []
            for env_info in info:
                if "metrics" in env_info:
                    each_rwd.append(env_info["metrics"])
            if len(each_rwd) > 0:
                print(f"\nEach metric: {np.array(each_rwd).mean(axis=0)}\n")
